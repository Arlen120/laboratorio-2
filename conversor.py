...
def convertir(precio_usd, moneda_destino, tasas):
    ...
    #obtiene la tasa de cambio e USD -> MONEDA_DESTINO
    tasa = tasas["USD"].get(moneda_destino)
    #si la ondea de destino no existe , lanza una excepcion
    if not tasa:
...
def registrar_transaccion(producto, precio_convertido, moneda, ruta_log):
    ...
    with open(ruta_log, "a") as archivo:
        #obtener la fecha actual con formato mes yyyy.mm.dd. HH:mm:ss 
        fecha = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        #Escribir una linea nueva en el archivo 
